...
function KrowiAF.GetFilteredAchievements_Old(categoryID)
    KrowiAF.Trace("KrowiAF.GetFilteredAchievements");

	local achievements = {};
	for _, achievement in next, KrowiAF.Categories_Old[categoryID].more.Achievements do
...
function KrowiAF.AchievementFrameAchievements_Update_Old()
    ...
    KrowiAF.Debug("KrowiAF.AchievementFrameAchievements_Update for category '" .. tostring(category) .. "' with name '" .. KrowiAF.Categories_Old[category].more.Name .. "'");

	local scrollFrame = AchievementFrameAchievementsContainer;
	local offset = HybridScrollFrame_GetOffset(scrollFrame);
    local buttons = scrollFrame.buttons;
	local achievements = KrowiAF.GetFilteredAchievements_Old(category);
	...
	for i = 1, #buttons do
		...
		if index > #achievements then
			...
			-- KrowiAF.AchievementButton_DisplayAchievement_Old(buttons[i], KrowiAF.Categories_Old[category].more.Achievements[index].ID, selection);
			AchievementButton_DisplayAchievement(buttons[i], KrowiAF.Categories_Old[category].more.Achievements[index].ID, nil, selection);
...
function KrowiAF.AchievementFrameAchievements_ForceUpdate_Old ()
	...
	KrowiAF.AchievementFrameAchievements_Update_Old();
end

function KrowiAF.AchievementButton_OnClick_Old(self, button, down, ignoreModifiers)
	...
	-- KrowiAF.AchievementButton_DisplayAchievement_Old(self, self.id, self.id);
...
function KrowiAF.AchievementButton_DisplayAchievement_Old(button, achievement, selectionID, renderOffScreen)
